 ...
 * @license This wScratchPad jQuery plug-in is dual licensed under the MIT and
 *          GPL licenses.
...
(function($) {
	...
	$.fn.wScratchPad.defaultSettings = {
		...
		color : '#336699', // set scratch color - if image2 is not set uses
		// color
		overlay : 'none', // set the type of overlay effect 'none', 'lighter'
		// - only used with color
	...
	ScratchPad.prototype = {
		generate : function() {
			...
			this.sp = $('<div></div>').css({
				position : 'relative'
					...
					$(this.canvas).attr('width', this.settings.width + 'px')
			...
			$(this.canvas).mousedown(function(e) {
				...
				if ($this.scratch)
					$this.scratchFunc(e, $this, 'Move');
			}).mouseup(function(e) {
				e.preventDefault();
				e.stopPropagation();

				// make sure we are in draw mode otherwise this will fire on any
				// mouse up.
				if ($this.scratch) {
					$this.scratch = false;
					...
					.bind(
							...
							function() {
								...
								simulatedEvent.initMouseEvent(type, true, true,
										window, 1, first.screenX,
										first.screenY, first.clientX,
										first.clientY, false, false, false,
										false, 0/* left */, null);
		...
		init : function() {
			...
			this.sp.css('cursor', (this.settings.cursor ? 'url("'
					+ this.settings.cursor + '"), default' : 'default'));

			$(this.canvas).css(
					{
						cursor : (this.settings.cursor ? 'url("'
								+ this.settings.cursor + '"), default'
								: 'default')
					});
		...
		scratchFunc : function(e, $this, event) {
			...
			if ($this.settings['scratch' + event])
				$this.settings['scratch' + event].apply($this, [ e,
						$this.scratchPercentage($this) ]);
		},

		scratchPercentage : function($this) {
			var hits = 0;
			var imageData = $this.ctx.getImageData(0, 0, $this.canvas.width,
					$this.canvas.height);
		...
		scratchDown : function(e, $this) {
			...
			// $this.ctx.beginPath();
			// $this.ctx.arc(e.pageX, e.pageY, $this.settings.size / 2, 0,
			// Math.PI * 2, true);
			// $this.ctx.closePath();
			// $this.ctx.fill();
